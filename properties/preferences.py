...
class GeneratorAddonPreferences(AddonPreferences):
    ...
    # --- Cineloom remote backend (P2 delta over Pallaidium) ----------------
    # When a "Cineloom Remote · …" model is selected in a dropdown, generation
    # is sent to this backend instead of running locally. Works against the
    # bundled Cineloom server or any OpenAI-compatible /v1 endpoint you run.
    # See models/remote_client.py.
    cineloom_remote_url: StringProperty(
        name="Remote Backend URL",
        description=(
            "Base URL of your remote generation backend (the bundled Cineloom "
            "server or any OpenAI-compatible /v1 endpoint), e.g. "
            "http://your-gpu-host:8879 . Used by the 'Cineloom Remote' models. "
            "Leave empty to disable remote generation."
        ),
        default="",
        maxlen=1024,
    )
    cineloom_remote_api_key: StringProperty(
        name="Remote API Key",
        description=(
            "API key for the remote backend (sent as Bearer / X-API-Key / "
            "?api_key). Optional for an open backend on a trusted network."
        ),
        default="",
        subtype="PASSWORD",
        maxlen=1024,
