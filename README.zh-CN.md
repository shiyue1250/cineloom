...
1. 在你的机器上装 Blender 5.2+（Windows / macOS / Linux 都行）
2. 克隆本仓库并安装插件：
   `Edit ▸ Preferences ▸ Add-ons ▸ Install from Disk` → 选仓库目录
   （或 `blender --command extension build` 构建 zip）
3. 在 **Edit ▸ Preferences ▸ Add-ons ▸ Cineloom** 中设置：
   - **Remote Backend URL** —— 例如 `http://你的后端:端口`
   - **API Key** —— 可选，以 `Bearer` / `X-API-Key` / `?api_key` 发送

远端插件代码只用 Python 标准库，所以桥接功能在任何系统上都无需额外依赖。
